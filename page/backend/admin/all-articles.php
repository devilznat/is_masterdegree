...
// echo $_SESSION["role_ID"];
  ...
  if(!isset($_SESSION['user_ID']))
  {
	...
	if($_GET["article_ID"]!="") {
		@unlink("../../../uploadfile/article/",$_GET["filename"]);
		$SQL = "delete from lawArticle where article_ID='".$_GET["article_ID"]."'";
...
//*** Update Last Stay in Login System
  ...
  $objResult = mysqli_fetch_array($objQuery,MYSQLI_ASSOC);
	...
	$SQL = "update lawArticle set value='' ";
	mysqli_query($link,$SQL) or die(mysqli_error($link).$SQL);

	for($i=1;$i<=3;$i++) {
		$SQL = "update lawArticle set value='".$i."' where article_ID='".$_POST["id".$i]."' ";
		mysqli_query($link,$SQL) or die(mysqli_error($link).$SQL);
	}
...
$SQL = "select * from lawArticle where value = '1' ";
      $rst1 = mysqli_query($link, $SQL) or die(mysqli_error($link).$SQL);
      $tmpcus = mysqli_fetch_array($rst1);
...
?>
    <div class="col-md-12 ">
      ...
      <div class="panel-title">กำหนดบทความสำหรับโชว์หน้า Index 
<br>  
      </div>
      <p>(โดยกรอกหมายเลขไอดีของบทความที่เลือกใส่ตามลำดับด้านล่างตามต้องการ) </p>
      <div class="panel-body table-responsive">
     

      <label for="input001" class="col-sm-2 control-label form-label">อันดับ 1</label>
      <div class="col-sm-10" style="width: 40%">
            <input type="text" class="form-control" id="input001" name="id1" width="40%"  value="<? echo $tmpcus["article_ID"]?>">
      </div><br><br>
       <?


      $SQL = "select * from lawArticle where value = '2' ";
      $rst1 = mysqli_query($link, $SQL) or die(mysqli_error($link).$SQL);
      $tmpcus = mysqli_fetch_array($rst1);


?>
      <label for="input002" class="col-sm-2 control-label form-label" >อันดับ 2</label>
      <div class="col-sm-10" style="width: 40%">
            <input type="text" class="form-control" id="input002" name="id2" width="40%" value="<? echo $tmpcus["article_ID"]?>">
      </div><br><br>
       <?

      $SQL = "select * from lawArticle where value = '3' ";
      $rst1 = mysqli_query($link, $SQL) or die(mysqli_error($link).$SQL);
      $tmpcus = mysqli_fetch_array($rst1);


?>
      <label for="input003" class="col-sm-2 control-label form-label">อันดับ 3</label>
      <div class="col-sm-10" style="width: 40%">
            <input type="text" class="form-control" id="input003" name="id3"  value="<? echo $tmpcus["article_ID"]?>">
      </div><br>
       
        <div class="col-xs-8">
           <button class="btn btn-default" type="submit" name="btnsub">บันทึก</button>
        </div>
    ...
    <div class="col-md-12 ">
      <div class="panel panel-widget">
        <div class="panel-title">
          บทความทั้งหมด<span class="label label-warning"><?php print $number1?></span>
...
<?php
		...
		$SQL = "select * from lawArticle order by article_ID desc $limit";
		$rst = mysqli_query($link, $SQL) or die(mysqli_error($link).$SQL);
...
?>
              ...
              <tr>
                ...
                <td><?php print $tmp2["typeTitle"]?></td>
                
                <td> 

                  <div class="btn-group" role="group" aria-label="...">
                  <a type="button" class="btn btn-warning" href="add-article.php?article_ID=<?php print $tmp["article_ID"]?>">แก้ไข</a> 
                  <a type="button" class="btn btn-danger"onclick="return remove_dw();" href="?article_ID=<?php print $tmp["article_ID"]?>&filename=<?php print $tmp["lawArticlePic"]?>">ลบ</a>
