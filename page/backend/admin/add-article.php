...
<html lang="en">
  ...
  <title>JustRooLaws | Consulting Website</title>
  ...
  <div class="page-header">
    ...
    <div class="right">
      <div class="btn-group" role="group" aria-label="...">
        <a href="dashboard_main" class="btn btn-light">Dashboard</a>
      ...
      <div class="panel panel-default">
		...
		<input type="hidden" name="filename" value="<?php print $tmp["lawArticlePic"]?>">
                ...
                <div class="form-group">
                  ...
                  <div class="col-sm-8" >
                    <select class="selectpicker" name="cate_ID" id="cate_ID" required>
					<option value=""></option>
			<?php
					$SQL = "select * from articleCategory order by cate_ID";
					$rst1 = mysqli_query($link, $SQL) or die(mysqli_error($link).$SQL);
					while(	$tmp1 = mysqli_fetch_array($rst1)) {
			?>
						<option value="<?php print $tmp1["cate_ID"]?>" <?php print ($tmp["cate_ID"]==$tmp1["cate_ID"])?"selected":""?>><?php print $tmp1["cateDetail"]?> (<?php print $tmp1["cateTitle"]?>)</option>
			<?php } ?>
                      </select>                  
                  </div>
                </div>
                  ...
                  <div class="col-sm-8">
                    <select class="selectpicker" name="type_ID" id="type_ID" required>
					<option value=""></option>
			<?php
					$SQL = "select * from articleType order by type_ID";
					$rst1 = mysqli_query($link, $SQL) or die(mysqli_error($link).$SQL);
					while(	$tmp1 = mysqli_fetch_array($rst1)) {
			?>
						<option value="<?php print $tmp1["type_ID"]?>" <?php print ($tmp["type_ID"]==$tmp1["type_ID"])?"selected":""?>><?php print $tmp1["typeDetail"]?> (<?php print $tmp1["typeTitle"]?>)</option>
			<?php } ?>
                      </select>                  
                  </div>
                </div>
			...
			<div class="form-group">
                ...
                <div class="col-sm-8" >
                    <select class="selectpicker" name="law_ID" id="law_ID" required>
					<option value=""></option>
			<?php
        ...
        <div class="panel-title">
          Summernote
            ...
            <div class="panel-body">
				...
				<br>
                <button class="btn btn-default" type="submit" onclick="return submit_new();">บันทึก</button>
...
<!-- ต้องวางสคริปต์ซ้อน เพราะวางอันเดียว กด select ไม่ขึ้น งงจ้า -->
<!-- ================================================
...
$('#lawArticleDetail').summernote({
  fontNames: ['rsuregular','Arial', 'Arial Black', 'Comic Sans MS', 'Courier New', 'Helvetica Neue' , 'Impact', 'Lucida Grande', 'Tahoma', 'Times New Roman',]
});
...
//   popover: {
//   image: [
//     ['imagesize', ['imageSize100', 'imageSize75', 'imageSize25']],
